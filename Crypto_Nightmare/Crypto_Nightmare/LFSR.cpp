...
bool LFSR::ReadBit(unsigned char target, int pos) {
	...
	unsigned char temp = target << pos;		// Shift higher order bits out.

	temp >>= (sizeof(temp) * 8 - 1);			// Shift desired bit to the ones place.

	if (temp == 1) {							// Check the value of the temporary value
		return true;							// and return the appropriate boolean value.
	}
	else {
		return false;
	}
